    ...
    if (this.value > 0)
    {
        ...
        $.post(url, {'region_id':this.value}, function(data){
                if (data)
                {
                    if (data.length > 0)
                    {
                        $("<select class='db_show_area form-control input-sm' style='width:100px;height:30px;line-height: 30px;margin-left: 5px;display: inline;'><option>"+AREA_SELECT_LANG+"</option></select>").change(regionChange).insertAfter(_self);
                        var data  = data;
